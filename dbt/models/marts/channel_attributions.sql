{{
    config(
        ...
        alias='mart_channel_attribution',
    )
}}
...
from
	{{ ref('fact_registrations') }} fr
left join {{ ref('dim_user') }} du on du.user_sk  = fr.user_sk 
left join {{ ref('fact_sessions') }} fs2 on fs2.user_sk =fr.user_sk 
left join {{ ref('dim_channel')}} dc on dc.channel_sk =fs2.channel_sk 
where 1=1
